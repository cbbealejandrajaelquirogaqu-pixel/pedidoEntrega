...
try:
    conn = mysql.connector.connect(
        host='Al3jael25.mysql.pythonanywhere-services.com',
        user='Al3jael25',
        password='Maracuya25'
    )

    # Mover el código del cursor DENTRO del try
    cursor = conn.cursor()
    cursor.execute("DROP DATABASE IF EXISTS `Al3jael25$pedidoEntrega`;")
...
cursor.execute("CREATE DATABASE `Al3jael25$pedidoEntrega`;")

cursor.execute("USE `Al3jael25$pedidoEntrega`;")
...
cursor.execute('select * from Al3jael25$pedidoEntrega.usuarios')
print(' -------------  Usuarios:  -------------')
...
cursor.execute('select * from Al3jael25$pedidoEntrega.productos')
print(' -------------  Productoss:  -------------')
